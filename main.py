...
def upload_df_with_batches(SQL, df, batch_size=500):
    ...
    warnings = []
    printProgressBar(0, length, prefix='Uploading dataframe to DB:', suffix='Complete', length=100)
    for i, batch in enumerate(batches):
        try:
            ...
            if 'Data truncated for column' in e.args[1]:
                ...
                print()
        conn.commit()
        printProgressBar(i+1, length, prefix='Uploading dataframe to DB:', suffix='Complete', length=100)
    conn.commit()
    close_db(cur, conn)
    if warnings:
